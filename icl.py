...
class PrototypeInContextLearner:
    ...
    def __init__(self, args):
        ...
        # Manually resolve hydra interpolations
    ...
    def _setup_model(self):
        ...
        # Initialize frozen backbone for domain fingerprinting
        ...
        if 'frozen_backbone_state_dict' in self.model_state:
            ...
            # Extract first layer from main backbone
            state_dict = {}
    ...
    def compute_domain_embedding(self, graph_data: Data) -> torch.Tensor:
        """Compute domain embedding for a new graph."""
    ...
    def compute_prototypes(self, features: torch.Tensor, labels: torch.Tensor, 
        ...
        """
        Compute class prototypes as mean of support features.
        
        Args:
            features: Feature vectors of support set [n_support, d]
            labels: Labels of support set [n_support]
            unique_labels: Unique class labels [n_classes]
            
        Returns:
            prototypes: Class prototype vectors [n_classes, d]
        """
        prototypes = []
    ...
    def prototype_inference(self, 
        ...
        """
        Perform prototype-based few-shot learning.
        ...
        Args:
            ...
            seed: Random seed
            distance_metric: 'euclidean' or 'cosine'
            use_label_prototypes: Whether to also use label embedding prototypes
        ...
        # Step 1: Compute domain embedding
        domain_embedding = self.compute_domain_embedding(graph_data)
        
        # Step 2: Compute FiLM parameters
        ...
        # Step 3: Get node embeddings using trained backbone
        ...
        # Step 4: Apply domain alignment
        z_all = gamma_f * H + beta_f
        z_all = F.normalize(z_all, p=2, dim=-1)
        
        # Step 5: Setup for few-shot learning
        labels = graph_data.y
        ...
        # Step 6: Compute class prototypes from support features
        ...
        # Optional: Combine with label embedding prototypes
        if use_label_prototypes and hasattr(self, 'E_label'):
            ...
            if E_label is not None:
                ...
                # Combine feature and label prototypes
        ...
        # Step 7: Classify test nodes based on prototype distance
