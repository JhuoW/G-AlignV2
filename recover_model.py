...
def recover_model_from_checkpoint():
    """Recover the model from checkpoint and save as final_gfm_model.pt"""
    ...
    # Choose the best checkpoint (lowest validation loss)
    ...
    # Load theta0 (initial parameters) for frozen backbone
    ...
    # Load cached domain embedding components
    if cfg.Fingerprint.DE_type == 'conv':
        ...
        # Load delta matrices for d_c_max
    ...
    # Prepare combined graphs info
    combined_graphs_info = {
        'num_nodes': 200761,  # From your data
        'num_features': 64,
        'num_graphs': 3,  # pubmed, arxiv, wikics
        'ptr': torch.tensor([0, 19717, 189060, 200761]),  # Cumulative node counts
        'name_dict': {'pubmed': 0, 'arxiv': 1, 'wikics': 2}
    }
    
    # Create the final model state dictionary
    final_model_state = {
...
if __name__ == "__main__":
    # Method 1: Recover from checkpoint (RECOMMENDED)
    logger.info("="*60)
